...
import { Search, HelpCircle, User, Check, X, Camera, CheckCircle, Clock } from 'lucide-react';
...
export default function ClientGalleryPage() {
  ...
  // Dummy filter categories for UI based on the image
  const filters = [
    { name: 'Semua', count: 120, active: true },
  ...
  const toggleSelectPhoto = (photoData) => {
    ...
    if (isAlreadySelected) {
      ...
      // Menyimpan object lengkap {name, url} agar bisa ditampilkan di sidebar
  ...
  const handleSaveSelection = async () => {
    setSaving(true);
    // Hanya menyimpan nama file (atau ID) ke database, bukan URL-nya
    ...
    <div className="min-h-screen bg-[#f7f7f7] font-sans text-gray-800 pb-12">
      ...
      <header className="bg-[#f7f7f7] border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm">
          ...
          <span className="text-gray-300 text-xs">▶</span>
          <div className="flex items-center gap-2 text-gray-400">
            <span className="bg-gray-200 text-gray-500 w-5 h-5 rounded-full flex items-center justify-center text-xs">3</span>
            Konfirmasi
          </div>
        </div>
      ...
      <main className="max-w-[1600px] mx-auto px-8 mt-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 max-w-2xl leading-relaxed">
            Tandai foto yang ingin kamu edit. Kamu bisa memilih beberapa foto sekaligus. Setelah selesai, lanjut ke tahap detail edit.
          </p>
        </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div className="flex gap-2">
                {filters.map((filter) => (
                  ...
                  >
                    {filter.name} <span className={`text-xs px-2 py-0.5 rounded-full ${filter.active ? 'bg-gray-700' : 'bg-gray-200'}`}>{filter.count}</span>
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredPhotos.map((photo) => {
                // Check if photo is in selectedPhotos array (comparing by name)
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 left-3">
                      <p className="text-white text-xs font-medium tracking-wide">{photo.name.replace(/\.[^/.]+$/, "")}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="mt-8 flex items-start gap-2 text-xs text-gray-500">
               ...
               <p>Kamu bisa memilih beberapa foto sekaligus. Jika ada pertanyaan, silakan hubungi kami melalui WhatsApp.</p>
            </div>
          </div>
                  ...
                  {selectedPhotos.length === 0 ? (
                    ...
                    selectedPhotos.map((photo) => (
                      <div key={photo.name} className="flex items-center gap-3">
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); removeSelectedPhoto(photo.name); }}
              ...
              <div className="p-2">
                ...
                <div className="flex flex-col gap-3 mb-6 text-sm text-gray-600">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-gray-900">{photos.length || 120}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><Clock size={16} className="text-gray-400"/> Maksimal (opsional)</div>
                ...
                <button 
                  ...
                  className="w-full bg-[#2a2a2a] text-white py-3.5 rounded-xl text-sm font-medium hover:bg-black transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mb-3"
                >
                  {saving ? 'Menyimpan...' : 'Lanjut ke Detail Edit'}
                  {!saving && <span className="text-lg leading-none">→</span>}
                </button>
